...
class SignupScreenCubit extends Cubit<SignupStates>{
 ...
 Future<String> lawerSignup({@required user_type,
  @required fname,
  @required email,
  @required password,
  ...
  @required phones1,
  @required national_id,})async{
    emit(SignupLoadingState());

   await DioHelper.PostData(url: SIGNUPLAWER,
        data: {
          'fname':fname,
          'lname':lname,
          'email':email,
          ...
          'date_of_birth': date_of_birth,
          'card_id':card_id,
          'card': await MultipartFile.fromFileSync(filePathCard,filename: fileNameCard),
  ...
  Future<String> uploadImage({@required user_type,
    @required fname,
    @required email,
    @required password,
    @required password_confirmation,
   ...
   var response = await DioHelper.PostData(url:SIGNUPLAWER, data:formData).then((value){
     emit(SignupSuccessState());
   });
