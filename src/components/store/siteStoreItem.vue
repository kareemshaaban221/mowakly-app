<template>
  ...
  <section id="stores" class="mt-5 mb-5" v-if="stores.length>=1">
    <div class="container">
...
export default {
  ...
  updated(){
       ...
       setInterval(() => {
        this.searchStore = sessionStorage.getItem("searchStore");
        // console.log(this.searchStore);
      }, 500);

      // // To clear the interval after a specific time, you can use setTimeout():
      // setTimeout(() => {
      //   clearInterval(intervalId);
      // }, 1000); // stop the interval after 5 seconds
  ...
  ,
    methods:{
        ...
        async getStores(){
          ...
          // if( this.searchStore ){
          //   let searchStoreQuery = this.searchStore
          // }

          await axios.get(`nearstores?lat=${localStorage.getItem('lat')}&long=${localStorage.getItem('lng')}&page=${this.currentPageP}&created_at=${this.createdAt}&nearest=${this.nearest}&region_id=${this.region_id}&city_id=${this.city_id}&search=${this.searchStore}`)
          .then( (res)=>{
    ...
    mounted(){
      ...
      // this.searchStore = localStorage.getItem('searchStore')
